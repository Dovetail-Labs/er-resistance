...
THREEcap.prototype.setupRecord = function () {
  this.video = new THREEcapVideo(this.settings);
}

THREEcap.prototype.cleanUp = function () {
  this.video.worker.terminate()
  this.video.worker = null;
}

THREEcap.prototype.record = function () {
  console.log('RECORD CALLED: ' + this.settings.width + 'x' + this.settings.height + '@' + this.settings.fps + ' for ' + this.settings.time + 's', this.settings);
  return new Promise(function (resolve, reject) {
    this.video.record(this.settings);
    this.video.on('finished', function (d) {
      console.log('FINISHED', this.video);
      resolve(this.video);
    }.bind(this));
  }.bind(this));
}
...
function THREEcapRenderPass(scriptbase) {
  var vertexShader = ['varying vec2 vUv;', 'void main() {', '	#ifdef FLIPPED', '		vUv = vec2( uv.x, 1.0 - uv.y );', '	#else', '		vUv = vec2( uv.x, uv.y );', '	#endif', '	gl_Position = projectionMatrix * modelViewMatrix * vec4( position, 1.0 );', '}'].join('\n');
  var fragmentShader = ['uniform float opacity;', 'uniform sampler2D tDiffuse;', 'varying vec2 vUv;', 'void main() {', '	vec4 texel = texture2D( tDiffuse, vUv );', '	gl_FragColor = opacity * texel;', '}'].join('\n')
  this.uniforms = {
    "tDiffuse": {
      type: "t",
      value: null
...
THREEcapVideo.prototype.device_input_close = function (stream) {
  ...
  this.processResults([{
    data: this.outputbuffer.subarray(0, this.outputpos)
  }]);
};
...
THREEcapVideo.prototype.record = function (settings) {
  ...
  if (this.useWorker) {
    ...
    var args = ['-framerate', fps.toString(), '-s', width + 'x' + height,];
    if (srcformat == 'raw') {
      args = args.concat(['-vcodec', 'rawvideo', '-f', 'rawvideo', '-pix_fmt', 'rgb24']);
    } else if (srcformat == 'png') {
      args = args.concat(['-f', 'image2pipe', '-vcodec', 'png', '-pix_fmt', 'rgb32']);
    }
    args = args.concat(['-i', '/input.raw',]);
    if (format == 'mp4') {
      args = args.concat(['-c:v', 'libx264', '-pix_fmt', 'yuv420p',
        //'-movflags', '+faststart',
        '-preset', quality,
      ]);
    } else if (format == 'gif') {
      args = args.concat(['-c:v', 'gif',]);
    } else if (format == 'raw') {
      args = args.concat(['-vcodec', 'rawvideo', '-pix_fmt', 'rgb24',]);
      format = 'mov';
    }
    args.push('-y');
