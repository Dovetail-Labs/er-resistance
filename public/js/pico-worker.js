...
addEventListener('message', prepare, {
  once: true
})
...
const memory = instantiate_detection_memory(5); // we will use the detecions of the last 5 frames
let regions = function(r, c, s, pixels, ldim) {
  return -1.0;
};
...
function prepare() {
  fetch(cascadeURL).then(function(response) {
    response.arrayBuffer().then(function(buffer) {
      ...
      regions = unpack_cascade(bytes);
      console.log('* cascade loaded');

      addEventListener('message', async (e) => {
        let dets = run_cascade(e.data, regions, params);
        dets = memory(dets);
        dets = cluster_detections(dets, 0.2); // set IoU threshold to 0.2
        for (i = 0; i < Math.min(1, dets.length); ++i) {
          // (the constant 50.0 is empirical: other cascades might require a different one)
          if (dets[i][3] > 50.0) {
            const mouthPositionX = dets[i][1];
            const mouthPositionY = (dets[i][0] + dets[i][2] / 4);
            postMessage({
              mouthPositionX,
              mouthPositionY
            });
